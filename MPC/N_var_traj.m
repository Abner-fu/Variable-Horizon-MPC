...
while(norm((x0-xs),2) > 1e-1 && current_time < sim_tim) % new - condition for ending the loop
    ...
    close_obs_flag_1 = curr_dist_1 <= kk1 && curr_dist_1 < last_dist_1;
    close_obs_flag_2 = curr_dist_2 <= kk2 && curr_dist_2 < last_dist_2;
    close_obs_flag_3 = curr_dist_3 <= kk3 && curr_dist_3 < last_dist_3;
    % close_obs_flag = curr_dist <= 0.9 && last_dist < curr_dist
    last_N = N;
    if close_obs_flag_1 && N ~= 16
        N = 16;
        [solver, args] = mpc_setup(N);
        if N > last_N
    ...
    if close_obs_flag_2 && N ~= ceil(16*obs2_size/obs1_size)
        N = ceil(16*obs2_size/obs1_size);
        [solver, args] = mpc_setup(N);
    ...
    if close_obs_flag_3 && N ~= ceil(16*obs3_size/obs1_size)
        N = ceil(16*obs3_size/obs1_size);
        [solver, args] = mpc_setup(N);
    ...
    if ~close_obs_flag_1 && ~close_obs_flag_2 && ~close_obs_flag_3 && N~=2
        N = 2;
        [solver, args] = mpc_setup(N);
...
save("data_traj.mat", "xx_var","-append");
