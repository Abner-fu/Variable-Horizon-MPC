function [solver, args] = mpc_setup(N)
...
% obstacle1.center = [2 4];
% obstacle1.radius = 0.3;
...
%     h1 = ((X(1,k)-obstacle1.center(1))^2+(X(2,k)-obstacle1.center(2))^2) - ((obstacle1.radius + rob_radius)^2);
%     h2 = ((X(1,k+1)-obstacle1.center(1))^2+(X(2,k+1)-obstacle1.center(2))^2) - ((obstacle1.radius + rob_radius)^2);
%     g = [g ; (1-gamma)*h1-h2];
% end
...
args.lbg(3*(N+1)+1 : 3*(N+1)+ N) = -inf; % inequality constraints
args.ubg(3*(N+1)+1 : 3*(N+1)+ N) = 0; % inequality constraints

args.lbx(1:3:3*(N+1),1) = 0; %state x lower bound % new - adapt the bound
